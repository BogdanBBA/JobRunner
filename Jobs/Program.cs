...
namespace Jobs
{
	class Program
	{
		...
		static void Finalize(JobRunResult result)
		{
			...
			JobLoggingDL DL = new JobLoggingDL();
		...
		{
			Const.Initialize(args[0]);

			if (args.Length < 2)
			{
				Log(LogLevel.Big, $"Incorrect console app call: arguments must be <\"binFolder\" jobID>.\n\nThis will not record any entries (it doesn't count as a run).", ConsoleColor.Magenta);
