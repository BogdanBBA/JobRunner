...
namespace CommonCode
{
    ...
    {
        private static Dictionary<Type, string> _defaultDictionary = new Dictionary<Type, string>
        {
            { typeof(int), "int" },
            { typeof(uint), "uint" },
            { typeof(long), "long" },
        ...
        public static string GetFriendlyName(this Type type, Dictionary<Type, string> translations)
        {
            if (translations.ContainsKey(type))
                ...
                return type.Name.Split('`')[0] + "<" + string.Join(", ", type.GetGenericArguments().Select(subtype => GetFriendlyName(subtype)).ToArray()) + ">";
            else
                return type.Name;
        }
